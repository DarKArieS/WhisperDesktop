...
void MelStreamer::makeTransposedBuffer( size_t off, size_t len )
{
   ...
   float mmax = 1e-20f;
	...
	{
		const MelChunk& src = queueMel[ i ];
		for( size_t j = 0; j < melCount; j++ )
		{
			const float value = src[ j ];
			outputMel[ j * len + i ] = value;
	...
	mmax -= 8.0f;
 for( float& value : outputMel )
	{
     value = std::max( value, mmax );
		value = ( value + 4.0f ) * ( 1.0f / 4.0f );
	}
}
	...
	if( availableMel < len )
	{
		...
		const size_t loop1 = std::min( missingMelChunks, pcmChunks );
		{
			...
			for( i = 0; i < loop1; i++ )
			{
                ...
                auto& arr = queueMel.emplace_back( melCount );
				...
				size_t availableFloats = availableChunks * FFT_STEP;
                 melContext.fft( &arr[ 0 ], sourcePcm, availableFloats );
			}
		}
		for( ; i < missingMelChunks; i++ )
		{
			assert( readerEof );
            auto& arr = queueMel.emplace_back( melCount );
            memset( &arr[ 0 ], 0, melCount * sizeof( float ) );
		}
	}
	...
	while( true )
	{
		...
		chunks = std::min( chunks, (ptrdiff_t)pcmChunks );
		{
			...
			if( this->workerThreads <= 1 || chunks < minChunksPerThread * 2 )
			{
				...
				{
                   MelChunk& arr = pendingChunks.emplace_back( melCount );
					const float* sourcePcm = tempPcm.data() + i * FFT_STEP;
					size_t availableChunks = pcmChunks - i;
					size_t availableFloats = availableChunks * FFT_STEP;
                  melContext.fft( arr.data(), sourcePcm, availableFloats );
				}
			}
			else
			{
              ...
              for( MelChunk& chunk : pendingChunks )
					chunk.resize( melCount );
...
HRESULT MelStreamerThread::threadPoolCallback( int ith ) noexcept
{
	...
	for( int i = i0; i < i1; i++ )
	{
		...
		size_t availableChunks = pcmChunks - i;
		size_t availableFloats = availableChunks * FFT_STEP;
     ctx.fft( &arr[ 0 ], sourcePcm, availableFloats );
	}
	return S_OK;
}
